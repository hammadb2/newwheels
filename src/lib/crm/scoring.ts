...
export function startingPriceCentsForTier(tier: LeadTier, verified = false): number {
  ...
  // Verified leads add a flat $150 premium across all tiers (per CEO decision
  // on PR #35). Verified hot = $350, verified warm = $300, verified standard = $250.
  const base = (() => {
    switch (tier) {
...
// a single source of truth. Flat $150 across all tiers.
